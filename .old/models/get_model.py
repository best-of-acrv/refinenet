...
def get_model(args, pretrained='imagenet'):
    ...
    if args.dataset.lower() == 'nyu':
        ...
        print('Invalid dataset chosen. Please choose from [nyu, voc, citiscapes]')
        exit()
    ...
    if args.model_type.lower() == 'refinenet':
        ...
        if args.num_resnet_layers == 50:
            ...
            model = refinenet101(num_classes=num_classes, pretrained=pretrained)
        elif args.num_resnet_layers == 152:
            model = refinenet152(num_classes=num_classes, pretrained=pretrained)
        else:
        ...
        if args.num_resnet_layers == 50:
            model = refinenet_lw50(num_classes=num_classes, pretrained=pretrained)
        elif args.num_resnet_layers == 101:
            model = refinenet_lw101(num_classes=num_classes, pretrained=pretrained)
        elif args.num_resnet_layers == 152:
            model = refinenet_lw152(num_classes=num_classes, pretrained=pretrained)
        else:
            print('Invalid number of ResNet layers chosen. Please choose from 50, 101 or 152 layers')
            exit()

    else:
        print('Invalid model type chosen. Please choose from [refinenet, refinenetlw]')
        exit()
